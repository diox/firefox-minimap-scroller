...
const { Cc, Cu, Ci } = require('chrome'),
    ...
    { BrowserWindow, browserWindows } = require('sdk/windows'),
...
var sidebars = {
    ...
    getActive: function(){
        let active = sidebars.getSidebar(browserWindows.activeWindow);
        return active || false;
    },

    changeVisibilityForAll: function(val) {
        sidebars.instances.forEach(function(item){
            item.visibility = val;
        });
    },

    changeWidthPref: function(prefName) {
    ...
    changePositionPref: function(prefName) {
        let value = prefs[prefName];
...
exports.SideBar = function(options) {
        ...
        onTrack: function (window) {
            ...
            if (defaultSidebar) {
                ...
                sidebar.init = function() {
                    ...
                    parseElements(clone);
                    if (options.title) {
                        sidebar.sidebarheader.firstChild.textContent = options.title;
                    }
                    if (options.header === false) {
                        sidebar.sidebarheader.style.display = 'none';
                    }
                    if (options.contextid) {
                        sidebar.browser.setAttribute('context', options.contextid);
                    }
                ...
                sidebars.instances.push(sidebar.init());
            }
        }
    ...
    let destroy = function() {
        while (sidebars.instances.length) {
            let item = sidebars.instances[0];
            item.destroy(); // Will automatically update the instances array.
        }
